...
On 2026-05-28 we validated that production clustering (384d HDBSCAN without UMAP) fails on Pipedrive data (silhouette 0.179, 3 meaningless clusters). Added UMAP-15d reduction to production pipeline (`backend/clustering/cluster.py`). Reclassified outreach strategies: extracted warm_intro from cold_template (event/referral/inbound), removed re_engagement. **Final: 5 strategies, 804 rows, silhouette 0.726, 6 clusters. Production and evaluation now use the same algorithm. 206 tests green.** Paper and presentation updated with new numbers, cluster visualization slide added (16 slides total). `papers/` added to .gitignore.
On 2026-05-28 (PM session) we ran chi-squared independence tests on 804 tier-1 rows: **cluster×outcome NOT significant (p=0.208)** — reply rates 72-83% too close within engaged cohort; **strategy×outcome HIGHLY significant (p≈0, V=0.318)** — cold_personal 97% vs feature_announcement 31%; **cluster×strategy confounded (p=0.00001)** — clusters received uneven strategy mixes. Conclusion: tier 2 data (24K no-reply) essential to disentangle cluster from strategy effect. Reorganized `evaluation/` into 4 levels (level0_data, level1_clustering, level2_policy, level3_learning). Fixed tier 2 extraction script (per-input state files, retry on transient errors). Tier 2 extraction started (762/24,415). Defy website analyzed (Vercel, pure HTML), deployment message drafted for CTO.
On 2026-05-31 supervisor reviewed the demo paper and flagged it as industry paper, not demo paper. Restructured to 4-section format (matching SOInspire template): Introduction with numbered system components (I-IV), The Demonstration, Preliminary Evaluation, Conclusion. Removed all equations, aligned writing style to abstract. Re-ran evaluation on doubled tier 2 data (13,806 rows, 744 clean): **cluster×outcome now V=0.346 (was 0.229)**, EDRA DR estimate 0.654 vs best single 0.588 (delta +0.066). Created architecture diagram HTML and 10-slide supplementary deck. **Seeded demo DB from 744 real outreach interactions** via `backend/seed_from_eval.py` — 7 clusters, 7 rules, 7 agents with real-world strategy distributions.
On 2026-06-01 (PM, second session) we addressed four issues surfaced from seeded-booth screenshots. (1) **Cluster-membership threshold "0.55 problem"**: diagnosed two confounded defects — clustering metric (UMAP+HDBSCAN) ≠ inference metric (raw single-NN cosine), AND seeded embeddings leaked the cluster label via a `(segment)` text suffix that inflated intra-cluster cosine, so the threshold was calibrated on the wrong reference distribution. Fixed: removed the suffix, switched the OOD gate to **cosine-to-cluster-centroid** (`select_rule_by_centroid`), recalibrated to **`MIN_CENTROID_SIMILARITY = 0.45`** (genuine-member p5; OOD outliers 0.26–0.28), role-display fallback `job_title → cluster_label → "Professional"`, reseeded anonymized. ⚠️ Only the *rejection* side is verified — no live in-domain profile tested against 0.45. (2) **Frontend rule-revision before→after diff** (baseline from `/state` `rulesById`, highlight only changed slots). (3) **Cluster-viz palette** 6→8 distinct dark-bg-readable colors (the scatter looked like noise because the palette couldn't separate 7 clusters, not because clustering was broken — t-SNE 2D is lossy). (4) **Honest induced rule revision** for the demo theater (the diff was empty because `proposed == current`): inject now adds a winning-alternative group + failing-current group, `should_revise` sorts by timestamp so the trigger window holds only failures, reflection sees both outcomes and induces the winner, deterministic mode-of-slots fallback guarantees a ≥2-slot diff offline. **252 tests green, 2 pre-existing cluster_viz failures. Booth NOT live-verified; all changes uncommitted.**
On 2026-06-01 (AM) we built a reversible **live rule-revision demo theater** (the revision UI existed but never fired on the seeded real-data DB) and fixed a wave of issues from a live booth run. New `POST /simulator/inject_contradiction` (defaults to largest ruled cluster) injects post-induction failing episodes bypassing `on_new_episode`, the consistency loop raises a pending revision, SSE streams live OpenAI reasoning, and a single **OK** preview button calls `POST /simulator/reset_injection` to roll everything back — real rules are never mutated. Fixed a latent `save_revision` datetime-JSON bug that would have crashed the loop. Six live-run fixes: rulebook cluster names from live cluster-viz; **out-of-distribution → no cluster → improvise** (`MIN_NEIGHBOR_SIMILARITY=0.55` in KNN, tuned on the seeded corpus); top-panel declutter to Episodes/Rules/Revising + a single Inject control; "Unclustered — improvising" label (+ backend stopped fabricating "The Visitor"); dynamic edge-handle bars (removed hardcoded "MAYA LIANG"/"DAY 03"); and the **second-conversation-won't-start** bug (cloned Fetch&Start button born `disabled`). Added local-only real names (`SEED_WITH_NAMES` env, default OFF — re-seed anonymized before deploy) and per-visitor email-gate reset. Mockup marked DEPRECATED; live frontend is source of truth. **248 tests green, 2 pre-existing cluster_viz failures, 0 new regressions. All changes left uncommitted pending commit decision.**
...
- [x] Restructured to 4-section demo paper format (2026-05-31): merged Intro+System, numbered components I-IV, removed equations, aligned style to abstract
- [x] Architecture diagram HTML created (`papers/edra_architecture_diagram.html`) — needs screenshot to PNG
- [x] 10-slide supplementary deck (`papers/edra_supplementary_slides.html`)
- [x] EvolveR and ReasoningBank bib entries filled with real author names
- [x] Demo video bib entry added (placeholder URL)
...
### Phase 12 — Seed demo from real data (done, 2026-05-31)
- [x] `backend/seed_from_eval.py` — seeds edra_lounge.db from 744 evaluation interactions
- [x] 7 clusters with centroid embeddings + labels from real CRM data
- [x] 7 rules (best strategy per cluster) with real reply rates
- [x] PII stripped (no names/emails), source_kind="synthetic"

## ✅ Phase 13 — Live rule-revision demo theater + live-run fixes (done, 2026-06-01)

See [[../sessions/2026-06-01 Live rule-revision demo theater and live-run fixes]].

### Demo theater (reversible)
- [x] `POST /simulator/inject_contradiction {cluster_id?, n?}` — injects `cs_window` rejected post-induction episodes into a cluster (default: largest ruled cluster), bypassing `on_new_episode` (no recluster). Consistency loop raises pending revision; SSE streams live OpenAI reasoning.
- [x] `POST /simulator/reset_injection` — deletes injected episodes + pending revision, restores rule to active. Idempotent, 409 guard if already resolved.
- [x] Frontend single **OK** button (replaced Accept/Edit/Keep) → `reset_injection`; "Preview… Not applied" framing. Real rules never mutated (pending-only design).
- [x] Fixed latent `save_revision`/`update_revision` datetime→JSON serialization bug (`model_dump(mode="json")`) that would have crashed the consistency loop.
- [x] Store: `delete_episodes`, `delete_revision`, `latest_revision_for_rule`; orchestrator tracks `injected_episode_ids`/`injected_rule_id`.

### Live-run fixes
- [x] Rulebook cluster names from live `/api/cluster-viz` (`clusterLabelById`), not hardcoded `ARCHETYPE_LABELS`.
- [x] Out-of-distribution → no cluster → improvise: `MIN_NEIGHBOR_SIMILARITY=0.55` in `select_rule_by_knn` (tuned on seeded corpus).
- [x] Top panel: Episodes/Rules/Revising + single Inject control (removed Day/Specialists/select/Reset/Segment/Expert; expert view always on).
- [x] "Unclustered — improvising" in CLASSIFIED ARCHETYPE (was leaking `profile.id`); backend `cluster_viz` stopped fabricating "The Visitor".
- [x] Dynamic edge-handle bars via `content: attr(data-label)` from live state (removed hardcoded "MAYA LIANG"/"DAY 03"/"R.07").
- [x] Second-conversation-won't-start: cloned Fetch&Start button born `disabled` → re-enable on dialog open; guarded stacked `setInterval` pollers.
- [x] Local-only real names: `SEED_WITH_NAMES` env (default OFF). ⚠️ **re-seed anonymized before deploy.**
- [x] Email gate reset between visitors (boot-gate retained); `bootAfterAuth` preloading idempotent.
- [x] `edra_pitch_mockup.html` marked DEPRECATED (CLAUDE.md + obsidian); live frontend is source of truth.

### Follow-ups (TODO)
- [ ] **Commit the accumulated work** in atomic groups: feature → live-run fixes (+ CLAUDE.md note) → obsidian separately; leave `papers/edra_demo.tex` (pre-existing) to the user.
- [ ] **Re-seed anonymized before deploy** — `python -m backend.seed_from_eval` (no `SEED_WITH_NAMES`).
- [ ] Fix the 2 pre-existing `test_cluster_viz` failures (archetype-label string mismatches).
...
- [ ] **BLOCKED: Aleks grants Editor role** on `active-brand-498516-d7` (daniel@defygroup.ai has no compute permissions)
- [ ] Create firewall rule + VM (e2-medium, europe-west1-b, Ubuntu 24.04)
