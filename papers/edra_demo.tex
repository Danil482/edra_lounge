...
We frame rule application as a contextual bandit and validate offline on 825 real cold-outreach interactions from a commercial CRM via a doubly robust estimator.
...
We frame EDRA's rule application as a contextual bandit: the context $x$ is a profile embedding, the action $a$ is one of three first-contact strategies, and the reward $r \in \{0,1\}$ indicates whether the recipient replied. We evaluate on 825 interactions extracted from a commercial CRM.

\noindent\textbf{Clustering Quality.}
HDBSCAN over MiniLM embeddings yields \textbf{[N]} clusters (silhouette = \textbf{[X.XXX]}). A $\chi^2$ test confirms that reply rates differ significantly across clusters ($p < $ \textbf{[X.XXX]}).

% ---- PLACEHOLDER: insert silhouette and chi-squared results ----

\noindent\textbf{Does Optimal Strategy Differ by Cluster?}
We compare four policies via the doubly robust estimator~\cite{dudik2011doubly}: $\pi_{\text{uniform}}$, $\pi_{\text{random}}$, $\pi_{\text{best}}$ (best single strategy for everyone), and $\pi_{\text{edra}}$ (cluster-conditional). If $\pi_{\text{edra}}$ cannot beat $\pi_{\text{best}}$, clustering adds nothing. Estimated rewards: \textbf{[X.XXX / X.XXX / X.XXX / X.XXX]}.
